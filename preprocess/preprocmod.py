...
def check_page(image):
	...
	if w<600: return "image too narrow for a page image %s"%(image.shape,) # original value = 600
...
def DSAVE(title,image):
	return #REMOVE LATER WHEN DEBUGGING
	global args
...
def compute_colseps_mconv(binary,scale=1.0):
	...
	seps = morph.select_regions(seps,sl.dim0,min=10*scale,nbest=2) #hardcoded arg vars
	DSAVE("3seps",seps)
...
def compute_gradmaps(binary,scale):
	...
	grad = gaussian_filter(1.0*cleaned,(max(4,1.0*0.3*scale),
											1.0*scale),order=(1,0))
...
def can_process(job):
	fname = job
	try:
		binary = ocrolib.read_image_binary(fname.encode('utf-8'))
	except IOError:
	...
	if isnan(scale) or scale>1000.0:
		return -1 
	if scale<12.0: #minscale
		return -1 
	segmentation = compute_segmentation(binary,scale)
	if amax(segmentation)>300: #maxlines
		return -1
	return 0
#
################################################################
...
def process1(job):
	...
	fname,i = job
	global base
	base,_ = ocrolib.allsplitext(str(fname))
	outputdir = base
	...
	try:
		...
		try:
			binary = ocrolib.read_image_binary(str(fname))
		except IOError:
			if ocrolib.trace: traceback.print_exc()
			print "cannot open either",base+".bin.png","or",fname
			return
	...
	ocrolib.write_page_segmentation(in_file,segmentation)#"%s.pseg.png"%outputdir
